...
public class HouseRaterizer implements WorldRasterizer {
    ...
    public void generateChunk(CoreChunk chunk, Region chunkRegion) {
        ...
        for (Map.Entry<BaseVector3i, House> entry : houseFacet.getWorldEntries().entrySet()) {
            Vector3i createHousePosition = new Vector3i(entry.getKey());
            int extent = entry.getValue().getExtent();
            createHousePosition.add(0, extent, 0);
            Region3i walls = Region3i.createFromCenterExtents(createHousePosition, extent);
            Region3i inside = Region3i.createFromCenterExtents(createHousePosition, extent - 1);
